...
The webpage describes how to build a tiny AI agent using **Model Context Protocol (MCP)** tools with **LangGraph**.

Here is a summary of the relevant concepts mentioned in the query that are covered in the text:

*   **agent\_infrastructure:** The core infrastructure discussed is an **MCP server** (for exposing tools) orchestrated by a **LangGraph** agent.
*   **tool use:** The example demonstrates an agent calling a specific tool (`add(a, b)`) exposed via an MCP server.
...
*   **function calling / structured outputs:** The MCP tool definition uses Python type hints (`a: int, b: int -> int`), and the SDK handles the structured invocation and return of results.
*   **agent memory / agentic memory / agent orchestration:** The "Production notes" section suggests that once the single-agent loop is solid, the next steps include adding **memory/checkpointing** and testing **multi-agent orchestration**.

**Concepts from the query NOT explicitly detailed or covered:**

*   MCP servers (only the concept and a minimal example are shown, not a deep dive into the infrastructure itself).
*   LlamaIndex
*   OpenAI Agents SDK (LangChain is used with OpenAI models, but not the specific SDK)
*   Anthropic Agents SDK (Anthropic is mentioned as a potential provider swap, but not detailed)
*   Google SDK
...
[Open in app](https://rsci.app.link/?$canonical_url=https://medium.com/p/58a1bd3e6616&amp;~feature=LoOpenInAppButton&amp;~channel=ShowPostUnderCollection&amp;~stage=mobileNavBar&amp;source=post_page---top_nav_layout_nav-----------------------------------------)
Sign up
...
[35K followers](https://ai.plainenglish.io/followers?source=post_page---post_publication_info--58a1bd3e6616---------------------------------------)
·[Last published12 hours ago](https://ai.plainenglish.io/ai-assisted-shopping-how-artificial-intelligence-is-transforming-the-2026-holiday-season-02f68a353614?source=post_page---post_publication_info--58a1bd3e6616---------------------------------------)
New AI, ML and Data Science articles every day. Follow to join our 3.5M+ monthly readers.
Follow
[
