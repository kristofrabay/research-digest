...
The Unified MCP Client Library (`mcp-use`) is an open-source library designed to connect **any LLM to any MCP server**, enabling the creation of custom agents with tool access.

Here is how the page content relates to your query:

*   **agent\_infrastructure: MCP servers, tool use, agent frameworks:** The library's core purpose is to connect LLMs to **MCP servers** to provide **tool access** for building custom agents. It explicitly supports integration with **LangChain** providers (like OpenAI and Anthropic) to facilitate agent creation.
*   **agent memory, agentic memory:** The page does not explicitly detail agent memory or agentic memory features within the `mcp-use` library itself, focusing instead on tool execution and orchestration.
*   **LangChain, LlamaIndex, OpenAI Agents SDK, Anthropic Agents SDK, Google SDK:** The library integrates directly with **LangChain** by requiring installation of provider-specific packages (e.g., `langchain-openai`, `langchain-anthropic`). It does not mention LlamaIndex, OpenAI Agents SDK, Anthropic Agents SDK, or Google SDK directly, though it supports LLMs from OpenAI and Anthropic via LangChain.
...
*   **agent orchestration:** The library supports orchestration through:
    *   **Multi-Server Support:** Configuring and connecting to multiple MCP servers simultaneously.
    *   **Dynamic Server Selection (Server Manager):** An option (`use\_server\_manager=True`) that allows the agent to intelligently select the correct MCP server based on the tool chosen by the LLM.
    *   **Tool Access Control:** Restricting which tools are available to the agent.

In summary, the page describes a library that acts as an infrastructure layer for building agents using MCP servers, integrating with LangChain, and supporting multi-tool orchestration.
...
](https://mcpservers.org/servers/tahooki/unreal-blender-mcp)[
...
](https://mcpservers.org/servers/gptprojectmanager/universal-infinite-loop-mcp-server)[
* ### VSCode MCP
Interact with VSCode through the Model Context Protocol, enabling AI agents to perform development tasks.
](https://mcpservers.org/servers/block/code-mcp)[
* ### Remote MCP Server (Authless)
An example of a remote MCP server without authentication, deployable on Cloudflare Workers.
](https://mcpservers.org/servers/mdhruvil/remote-mcp-server-authless)[
* ### MCP Options Order Flow Server
A high-performance MCP server for comprehensive options order flow analysis.
](https://mcpservers.org/servers/fintools-ai/mcp-options-order-flow-server)[
* ### MCPHost
A CLI host application that enables Large Language Models (LLMs) to interact with external tools through the Model Context Protocol (MCP).
](https://mcpservers.org/servers/mark3labs/mcphost)[
* ### MCPatterns
...
](https://mcpservers.org/servers/p10q/ae-mcp)[
...
](https://mcpservers.org/servers/BetterThanTomorrow/calva-backseat-driver)[
* ### Mentor MCP
