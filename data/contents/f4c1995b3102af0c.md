...
The provided webpage focuses on **Debugging MCP Servers** using the **MCP Inspector** tool, which is part of the **Model Context Protocol (MCP) Framework**.

The summary of the page relevant to the query is:

The page details how to use the **MCP Inspector** to test and debug **MCP servers**. This tool provides a user interface for interacting with the server, testing its **tools**, **resources**, and **prompts**. It covers:
*   How to run the Inspector via `npx`.
*   Customizing the client UI and proxy server ports.
*   Features for testing **tools** (viewing schemas, testing inputs) and inspecting **resources**.
*   Testing **prompts**.
*   Using the built-in **Framework Logging** capabilities which integrate with the Inspector.
*   A general development workflow and troubleshooting common issues (like "Tool Not Found").

**No answer found** for the majority of the terms in your query, such as `agent_infrastructure`, `tool use`, `deep research`, `agent memory`, `agentic memory`, `context compression`, `agent frameworks`, `LangChain`, `LlamaIndex`, `OpenAI Agents SDK`, `Anthropic Agents SDK`, `Google SDK`, `function calling`, `structured outputs`, `agent orchestration`, `agent evaluations`, `LLM reasoning trace evaluation`, `prompt engineering`, `context engineering`, `long context`, `supervised fine-tuning`, and `reinforcement learning`. The page is specifically about debugging the MCP Framework, not general agent infrastructure or advanced LLM techniques.
