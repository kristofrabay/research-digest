...
This page focuses on the **Information Retrieval Phase** of a Retrieval-Augmented Generation (RAG) solution, covering how to configure a search index, choose search approaches, and refine queries.
...
Develop a RAG Solution—Information-Retrieval Phase - Azure Architecture Center | Microsoft Learn
...
Table of contents[Read in English](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-information-retrieval)Add to CollectionsAdd to plan[Edit](https://github.com/microsoftdocs/architecture-center/blob/main/docs/ai-ml/guide/rag/rag-information-retrieval.md)
#### Share via
[Facebook](https://www.facebook.com/sharer/sharer.php?u=https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-information-retrieval?WT.mc_id=facebook)[x.com](https://twitter.com/intent/tweet?original_referer=https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-information-retrieval?WT.mc_id=twitter&amp;tw_p=tweetbutton&amp;url=https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-information-retrieval?WT.mc_id=twitter)[LinkedIn]()[Email]()
Print
Note
...
This article is part of a series. Read the[introduction](rag-solution-design-and-evaluation-guide).
[](#configure-your-search-index)## Configure your search index
Note
...
The search index in your store has a column for every field in your data. Search stores generally support[nonvector data types](https://learn.microsoft.com/en-us/rest/api/searchservice/supported-data-types#edm-data-types-for-nonvector-fields), such as string, boolean, integer, single, double, and datetime. They also support collections, such as single-type collections and[vector data types](https://learn.microsoft.com/en-us/rest/api/searchservice/supported-data-types#edm-data-types-for-vector-fields). For each column, you must[configure information](https://learn.microsoft.com/en-us/azure/search/vector-search-how-to-create-index#add-a-vector-field-to-the-fields-collection), such as the data type and whether the field is filterable, retrievable, or searchable.
Consider the following[vector search configurations](https://learn.microsoft.com/en-us/azure/search/vector-search-how-to-create-index#add-a-vector-search-configuration)that you can apply to vector fields:
* **Vector search algorithm:**The[vector search algorithm](https://learn.microsoft.com/en-us/azure/search/vector-search-ranking)searches for relative matches. AI Search has a brute-force algorithm option, called exhaustive k-nearest neighbors (KNN), that scans the entire vector space. It also has a more performant algorithm option, called Hierarchical Navigable Small World (HNSW), that performs an[approximate nearest neighbor (ANN)](https://learn.microsoft.com/en-us/azure/search/vector-search-overview#approximate-nearest-neighbors)search.
* **Similarity metric:**The algorithm uses a[similarity metric](https://learn.microsoft.com/en-us/azure/search/vector-search-ranking#similarity-metrics-used-to-measure-nearness)to calculate nearness. The types of metrics in AI Search include cosine, dot product, and Euclidean. If you use Azure OpenAI Service embedding models, choose cosine.
...
[](#choose-your-search-approach)## Choose your search approach
...
[Vector searches](https://learn.microsoft.com/en-us/azure/search/vector-search-how-to-query)compare the similarity between the vectorized query (prompt) and vector fields. For more information, see[Choose an Azure service for vector searches](../../../guide/technology-choices/vector-search).
Important
Before you embed the query, you should perform the same[cleaning operations](rag-enrichment-phase#clean-your-data)that you performed on chunks. For example, if you lowercased every word in your embedded chunk, you should lowercase every word in the query before embedding.
Note
...
PythonCopy
```
`embedding = embedding\_model.generate\_embedding(
chunk=str(pre\_process.preprocess(query))
)
vector = RawVectorQuery(
k=retrieve\_num\_of\_documents,
fields="contentVector",
vector=embedding,
)
results = client.search(
search\_text=None,
vector\_queries=[vector],
top=retrieve\_num\_of\_documents,
...
[Full-text searches](https://learn.microsoft.com/en-us/azure/search/search-lucene-query-architecture)match plain text that's stored in an index. It's common practice to extract keywords from a query and use those extracted keywords in a full-text search against one or more indexed columns. You can configure full-text searches to return matches if any terms or all terms match.
Experiment to determine which fields to run full-text searches against. As described in the[enrichment phase article](rag-enrichment-phase#augment-your-chunks), you should use keyword and entity metadata fields for full-text searches in scenarios where content has similar semantic meaning but entities or keywords differ. Other common fields to consider for full-text search include title, summary, and chunk text.
The following sample code performs a full-text search against the title, content, and summary fields.
PythonCopy
```
`formatted\_search\_results = []
results = client.search(
search\_text=query,
top=retrieve\_num\_of\_documents,
select=["title","content","summary"],
)
formatted\_search\_results = format\_results(results)`
```
[](#hybrid-search)#### Hybrid search
AI Search supports[hybrid queries](https://learn.microsoft.com/en-us/azure/search/hybrid-search-ranking)that contain one or more text searches and one or more vector searches. The platform performs each query, gets the intermediate results, reranks the results by using[Reciprocal Rank Fusion](https://learn.microsoft.com/en-us/azure/search/hybrid-search-ranking#how-rrf-ranking-works), and returns the top*N*results.
The following sample code performs a full-text search against the title, content, and summary fields. It also performs vector searches against the contentVector and questionVector fields. AI Search runs all the queries in parallel, reranks the results, and returns the top*retrieve\_num\_of\_documents*.
PythonCopy
```
`embedding = embedding\_model.generate\_embedding(
chunk=str(pre\_process.preprocess(query))
...
results = client.search(
search\_text=query,
vector\_queries=[vector1, vector2],
top=retrieve\_num\_of\_documents,
select=["title","content","summary"],
)`
```
[](#manual-multiple-queries)#### Manual multiple queries
...
```
[](#the-hyde-technique)#### The HyDE technique
...
[](#combine-query-translations-into-a-pipeline)### Combine query translations into a pipeline
...
[](#pass-images-in-queries)### Pass images in queries
...
[](#language-model-reranking)#### Language model reranking
...
{
"documents": {
"document\_1": "Relevance",
"document\_2": "Relevance"
}
}`
```
...
PythonCopy
```
`fromsentence\_transformersimportCrossEncoder
...
model\_name ='cross-encoder/stsb-roberta-base'model = CrossEncoder(model\_name)
cross\_scores\_ques = model.predict(
[[user\_prompt, item]foritemindocuments],
apply\_softmax=True,
convert\_to\_numpy=True,
)
top\_indices\_ques = cross\_scores\_ques.argsort()[-k:][::-1]
sub\_context = []foridxinlist(top\_indices\_ques):
sub\_context.append(documents[idx])`
```
[](#semantic-ranking)#### Semantic ranking
AI Search has a proprietary feature called[semantic ranking](https://learn.microsoft.com/en-us/azure/search/semantic-search-overview). This feature uses deep learning models that were adapted from Microsoft Bing that promote the most semantically relevant results. For more information, see[How semantic ranker works](https://learn.microsoft.com/en-us/azure/search/semantic-search-overview#how-semantic-ranker-works).
[](#consider-other-search-guidance)### Consider other search guidance
...
[](#evaluate-your-search-results)## Evaluate your search results
In the preparation phase, you[gathered test queries along with test document information](rag-preparation-phase#gather-test-query-output). You can use the following information that you gathered in that phase to evaluate your search results:
...
[](#next-step)## Next step
[LLM end-to-end evaluation phase](rag-llm-evaluation-phase)
[](#related-resources)## Related resources
* [Quickstart: Chat with Azure OpenAI models by using your own data](https://learn.microsoft.com/en-us/azure/ai-services/openai/use-your-data-quickstart)
* [Hybrid search via vectors and full text in AI Search](https://learn.microsoft.com/en-us/azure/search/hybrid-search-overview)
## Feedback
...
Documentation
* [Develop a RAG Solution - Chunk Enrichment Phase - Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-enrichment-phase?source=recommendations)
Learn how to augment your chunks with metadata like title, summary, keywords, entities, and more and learn common cleaning methods that can improve vector matches.
* [Develop a RAG Solution - Chunking Phase - Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-chunking-phase?source=recommendations)
Learn about the various chunking strategies like boundary based, custom code, and document analysis models. Also learn about how the document structure should influence your chunking strategy.
* [Develop a RAG Solution - Generate Embeddings Phase - Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-generate-embeddings?source=recommendations)
Learn how embeddings work, how to choose an embedding model, and how your embedding model can affect your vector search results.
* [Develop a RAG Solution - Preparation Phase - Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-preparation-phase?source=recommendations)
Learn how to analyze and gather representative content such as documents, images, videos, and audio files and develop test queries to validate your RAG solution.
* [Develop a RAG Solution - Large Language Model End-to-End Evaluation Phase - Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-llm-evaluation-phase?source=recommendations)
Learn about how to evaluate your RAG solution from end to end by calculating and evaluating key large language model and similarity metrics.
* [Design and Develop a RAG Solution - Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/rag/rag-solution-design-and-evaluation-guide?source=recommendations)
...
* [Design a Secure Multitenant RAG Inferencing Solution - Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/secure-multitenant-rag?source=recommendations)
Learn about the considerations and recommendations for improved security that you should consider when you design a multitenant RAG inferencing solution.
Show 4 more
Training
Module
