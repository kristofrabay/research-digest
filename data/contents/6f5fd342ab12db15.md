...
*   **Vector Databases and Search Index:** Discusses the crucial role of search indexes, including vector indexes (like those using HNSW) and managed solutions like vector databases (Pinecone, Weaviate, Chroma).
*   **Embeddings (New Efficient Models):** Mentions choosing search-optimized embedding models (e.g., bge-large, E5 family) and touches upon **Encoder fine-tuning** to improve embedding quality.
*   **Chunking Strategies:** Details the necessity of chunking data and discusses the trade-off in chunk size, referencing LlamaIndex's `NodeParser`. Advanced strategies include **Sentence Window Retrieval** and **Auto-merging Retriever** (Parent Document Retriever), which retrieve small chunks but provide larger context to the LLM.
*   **RAG Architectures & Alternatives:** Covers various advanced architectures like **Hierarchical indices**, **Hypothetical Questions and HyDE**, **Fusion retrieval or hybrid search** (combining keyword search like BM25 with vector search using Reciprocal Rank Fusion), **Query transformations** (like Multi Query Retriever and Step-back prompting), **Query Routing**, and **Agents in RAG**.
*   **Rerankers:** Discusses refining retrieval results using **Postprocessors** which can filter results or **rerank** them using models like cross-encoders or LLMs. It also mentions **Ranker fine-tuning**.
*   **RAG Architectures (General):** The article systematically breaks down the RAG pipeline, from chunking/vectorization to search, reranking/filtering, query transformation, chat engines, routing, agents, and response synthesis.
...
·[Last publishedjust now](https://pub.towardsai.net/hybrid-search-demystified-how-to-combine-vector-and-keyword-search-like-a-pro-d955612cd634?source=post_page---post_publication_info--04d193d8fec6---------------------------------------)
Making AI accessible to 100K+ learners. Find the most practical, hands-on and comprehensive AI Engineering and AI for Work certifications at[academy.towardsai.net](http://academy.towardsai.net)- we have pathways for any experience level. Monthly cohorts still open — use COHORT10 for 10% off!
Follow
[
