...
**Published:** 2025-09-17T06:56:42.000Z

---

## Summary

The webpage provides a detailed guide on understanding and implementing agentic AI using **LangChain**, **LangGraph**, and **LlamaIndex**.

The summary relevant to your query is as follows:

*   **Agent Frameworks (LangChain, LangGraph, LlamaIndex):** The article extensively covers the foundational building blocks and practical implementation of these three major agent frameworks, showing how to create chains (LangChain), graphs (LangGraph), and workflows (LlamaIndex).
*   **Tool Use (Function Calling):** It demonstrates how to integrate external capabilities using **Tools**. Specifically, it shows how to use the **Tavily** search API as a tool, and how to define custom tools (like `internal_lookup`) for an agent to decide when and how to use them (Tool Calling).
*   **Agent Memory:** The article explains how to add conversational memory to both LangChain (using `RunnableWithMessageHistory`) and LangGraph (using `MemorySaver`) to allow agents to retain context across interactions.
*   **Structured Outputs/Function Calling:** The LangChain section shows how to use `@tool` decorators for function calling, allowing the LLM to decide which function to invoke with specific arguments.
*   **Agent Orchestration:** LangGraph is presented as a powerful tool for orchestration, using conditional edges (`add_conditional_edges`) to manage complex flows, such as deciding whether to proceed to a tool execution node or end the process. LangChain's `initialize_agent` and LlamaIndex's `ReActAgent` are also shown as higher-level orchestration methods.
*   **MCP Servers, Agentic Memory, Google SDK, Anthropic Agents SDK, OpenAI Agents SDK:** These specific terms were **not** explicitly discussed or detailed in the provided text. The article focuses primarily on LangChain, LangGraph, and LlamaIndex components, using OpenAI and Mistral models.

**In conclusion, the page covers agent infrastructure components like tool use, memory, frameworks (LangChain, LangGraph, LlamaIndex), function calling, and orchestration, but it does not mention MCP servers, agentic memory (beyond standard chat history), or the specific SDKs for Anthropic or Google.**
...
[285 followers](https://medium.com/mitb-for-all/followers?source=post_page---post_publication_info--5d470fb51e29---------------------------------------)
